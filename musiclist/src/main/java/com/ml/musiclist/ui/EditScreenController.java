...
import com.ml.musiclist.helper.HyperlinkCell;
import com.ml.musiclist.helper.TableMediaLink;
import com.ml.musiclist.helper.TableSong;
import com.ml.musiclist.service.AlbumService;
import com.ml.musiclist.service.ApiService;
...
public class EditScreenController implements Initializable {
    ...
    @FXML
    private TextField mbidInputField;
    @FXML
    private Button findCoverButton;
    @FXML
    ...
    public void setData(long albumId) {
        ...
        if (album.getAdditionDate() != null) {
            addDate = album.getAdditionDate();
        } else {
    ...
    public void handleCoverImageClicked (MouseEvent event) {
        System.out.println("Cover clicked");
        ...
        try {
            ...
            if (originalFile.toURI().compareTo(newFile.toURI()) != 0) {
                System.out.println(originalFile.toURI() + " != " + newFile.toURI());
    ...
    public void handleSaveChangesButton (ActionEvent event) {
        ...
        editedAlbum.setTitle(titleInputField.getText());
        editedAlbum.setListenedTo(listenedCheckBox.isSelected());
        if (listenedCheckBox.isSelected()) {
            editedAlbum.setListenedDate(LocalDate.now());
        }
        editedAlbum.setOwned(ownedCheckBox.isSelected());
        editedAlbum.setCountry(countryInputField.getText());
        editedAlbum.setAdditionDate(addDate);
        editedAlbum.setNotes(noteInputField.getText());
        ...
        editedAlbum.setFormat(albumFormat);
        ...
        editedAlbum.setType(albumType);
        editedAlbum.setReleaseDate(releaseDatePicker.getValue());
        editedAlbum.setLength((int)albumLength.getSeconds());
        editedAlbum.setArtists(artistsToAdd);
        editedAlbum.setGenres(genresToAdd);
        editedAlbum.setLanguages(langsToAdd);
        editedAlbum.setTags(tagsToAdd);
        editedAlbum.setMediaLinks(mediaToAdd);
        editedAlbum.setSongs(songsToAdd);
        editedAlbum.setCoverPath(coverPath);
    ...
    public void handleFindCoverButton (ActionEvent event) {
        if (!mbidInputField.getText().isEmpty() || !mbidInputField.getText().isBlank()) {
            statusLabel.setText("Searching...");
            String mbid = mbidInputField.getText().trim();
            try (ExecutorService executorService = Executors.newCachedThreadPool()){
                Task<String> task = new Task<String>() {
                    @Override
                    protected String call() throws Exception {
                        return apiService.getCover(mbid);
                    }
                };
    ...
    private void setButtonHandlers() {
        saveChangesButton.setOnAction(this::handleSaveChangesButton);
        ...
        findCoverButton.setOnAction(this::handleFindCoverButton);
    }
    private void setupTextFields() {
        ...
        songNumberField.textProperty().addListener(new ChangeListener<String>() {       //set as numeric input only
            @Override
            public void changed(ObservableValue<? extends String> observable, String oldValue,
                                String newValue) {
                if (!newValue.matches("\\d*")) {
                    songNumberField.setText(newValue.replaceAll("[^\\d]", ""));
                }
            }
        });
    }
