...
public interface AlbumRepository extends JpaRepository<Album, Long> {
    ...
    @Query ("SELECT new com.ml.catalogg.dbcount.ArtistCount(art.name, count(alb) ) " +
            "FROM Album alb" +
    ...
    @Query ("SELECT new com.ml.catalogg.dbcount.GenreCount(gen.name, count(alb) ) " +
            "FROM Album alb" +
            " JOIN alb.genres gen" +
            " WHERE alb.listenedTo = true AND alb.listenedDate > :date" +
            " GROUP BY gen.name")
    ...
    @Query ("SELECT new com.ml.catalogg.dbcount.TagCount(tg.tag, count(alb) ) " +
            "FROM Album alb" +
            " JOIN alb.tags tg" +
            " WHERE alb.listenedTo = true AND alb.listenedDate > :date" +
            " GROUP BY tg.tag")
